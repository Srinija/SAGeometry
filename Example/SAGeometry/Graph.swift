...
class Graph: UIView {
    ...
    func setTransform(minX: CGFloat, maxX: CGFloat, minY: CGFloat, maxY: CGFloat) {
        ...
        let xScale = (screenWidth - yOffset - 10)/(maxX - minX)
        let yScale = ((screenHeight - xOffset - 30) - (screenHeight - xOffset - 30).truncatingRemainder(dividingBy: 10))/(maxY - minY)
        
        chartTransform = CGAffineTransform(a: xScale, b: 0, c: 0, d: -yScale, tx: yOffset, ty: screenHeight - xOffset)
